...
module.exports = class UserService extends Schmervice.Service {
    ...
    async update(id, { password, ...userInfo }, txn) {
        ...
        if (password) {
            const user = await this.findById(id, txn);
            await passwordCheck(password_current, user, txn);
        }
    ...
    async login({ email, password }, txn, response) {
        ...
        response.state("token", token, this.options.cookie_options);
